...
export default function RouteInfo({ data, loading, mapsUrl }: { data: RouteResult | null; loading: boolean; mapsUrl: string }) {
  return <div className="text-xs text-neutral-500 space-y-1 my-3">
    {loading ? <p role="status">Loading route information...</p> : data ? <>
      ...
      <p>{data.trafficAwareDurationSeconds !== null ? 'Traffic-aware driving estimate' : 'Driving estimate · traffic unavailable'}
        {data.timing === 'scheduled' ? ' for ' + new Date(data.departureTime).toLocaleString() : ' for departure now'}</p>
      <p>Google Maps · Updated {new Date(data.calculatedAt).toLocaleTimeString()}</p>
